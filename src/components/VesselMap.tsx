...
function FlotillaCenter({ 
  ...
  // Haversine formula to calculate distance in nautical miles - memoized
  ...
  // Function to calculate a point at a given distance along a line - memoized
  const calculatePointAtDistance = useCallback((lat1: number, lng1: number, lat2: number, lng2: number, distanceNm: number): { lat: number; lng: number } => {
...
function ZoomDependentProgressDots({ 
  ...
  // Memoize progress dots calculations
  const progressDots = useMemo(() => {
    if (!flotillaData || !flotillaData.forwardVessel || !flotillaData.distance || !flotillaData.averageSpeed) {
      return [];
    }

    const dots = [
      { name: 'Y2', distance: 233.3, color: '#eab308' },
      { name: 'Y1', distance: 166.7, color: '#eab308' },
      { name: 'R3', distance: 100, color: '#ef4444' },
      { name: 'R2', distance: 66.6, color: '#ef4444' },
      { name: 'R1', distance: 33.3, color: '#ef4444' }
    ];

    return dots
      .filter(dot => flotillaData.distance! > dot.distance)
      .map(dot => {
        const position = calculatePointAtDistance(
          GAZA_PORT.lat, GAZA_PORT.lng,
          flotillaData.forwardVessel!.lat, flotillaData.forwardVessel!.lng,
          dot.distance
        );
        
        const distanceToDot = calculateDistance(
          flotillaData.forwardVessel!.lat, flotillaData.forwardVessel!.lng,
  ...
  return (
    <>
      {/* Render all progress dots from memoized calculations */}
      {progressDots.map((dot) => (
        <Marker
          key={dot.name}
          position={[dot.position.lat, dot.position.lng]}
          icon={L.divIcon({
            html: `
              <div style="
                display: flex;
                              ...
                              <div className="space-y-2">
                                ...
                                {(() => {
                                  ...
                                  if (participants.length > 0) {
                                    return (
                                      <div className="space-y-0.5">
                                        <div className="text-xs text-slate-400 font-mono uppercase tracking-wider flex items-center gap-1">
                                          <img src="/MY.png" alt="Malaysia" className="w-3 h-3" />
                                          MALAYSIAN PARTICIPANTS
                                        </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="font-medium text-sm text-slate-800 dark:text-slate-200 truncate flex items-center gap-1">
                                ...
                                {getMalaysianParticipants(vessel.name).length > 0 && (
                                  <img src="/MY.png" alt="Malaysia" className="w-4 h-4" />
                                )}
